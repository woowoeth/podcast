...
: "${ONLY:=}"
: "${ONLY_RESIDENTIAL:=}"
# **本机线的并发要比云端低。** 转写被 _ASR_LOCK 串成一条，多开的那几个
# worker 只能做 LLM 那几段 —— 有用（转写一集的时候另外几集在做深读／评审），
# 但再多就只是排队。4 够用。
: "${JOBS:=4}"
export JOBS
: "${LIMIT:=2}"
...
# 30000 装不下两三小时的长访谈：AXRP 一集 31,755 词，每集都这样 ——
...
PYEOF
  ...
  #
  # **预算按机器的实际空闲来定，不按习惯来。** 量过三件事：
  #   · 抓 165 档 feed + 筛候选 只要 15 秒，手上常年有 400 条候选 —— 发现不是瓶颈；
  #   · 本地转写 12.2× 实时（56 分钟音频 274 秒），一集约 5 分钟，是瓶颈；
  #   · 并行转写没用：同一段音频 1 路 20.7 秒、2 路 21.3、3 路 33.7 —— GPU 已经吃满，
  #     _ASR_LOCK 拦得对，多开只会互相拖慢。
  # 而这台机器一天只跑两轮、每轮占 25 分钟，**GPU 一天闲 23 小时**。
  # 瓶颈拆不动的时候，能拿的只有那些没在用的时间。
  ...
  # 转写，走字幕的一分钱不花。
  # **每周一趟，不是每天。** 云端 IP 吃不到字幕，验字幕+入库只能在本机这条线；
  # 但观察名单变动很慢，而每天跑的代价是实打实的：
  #   · 名单里近百个频道，每个要逐支视频取字幕直到命中（yt-dlp，慢且招限流）；
  #   · 被评过分又没收的频道，**每天会被重新评一次**——curate 只对已在册的
  #     去重，评过但没收的不记账，那是重复的模型钱。
  # 用 ISO 周数当戳（date +%G%V），一周只落一次；周一跑，跑过就跳过。
