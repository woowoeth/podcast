...
def judge(sid: str, m: dict) -> tuple[str, str] | None:
    """返回 (动作, 理由)；无需动作时返回 None。动作: drop | demote | dormant"""
    if m["feed_ok"] is False:
        streak = m.get("fail_streak") or 1
        if streak < DEAD_STREAK:
            return None     # 一两次失败是抖动：YouTube 会限流返 404、Substack 会 403
...
def apply_actions(actions: list[tuple[str, str, str]], dry: bool = False) -> list[dict]:
    """改 sources.json 的 tier / 移除条目，并生成日志条目。"""
...
def apple_charts(limit: int = 30) -> list[dict]:
    """Apple 分类榜。有实据的候选池——榜上的节目一定真实存在且当前活跃。"""
    out, seen = [], set()
...
SHORTLIST_SYSTEM = """你在给一个中文播客深读站从榜单里挑候选。读者是中国的创业者、
投资人和工程师，他们要的是可核对的判断、机制和数字。

只凭节目名做粗筛，把明显不值得的剔掉即可，不确定的留下（后面还有实测和详细打分）。

明显不要的：个人成长／励志／效率鸡汤、销售话术、加密币喊单、日播新闻综述、
泛泛的商业访谈秀、宗教、健康养生、语言学习、以及任何看名字就与科技／产业／投资无关的。

值得留的：具体领域的深访或机制拆解（半导体、算力、基础设施、支付、监管、
产业链、一手创业与投资经验）。"""
...
def shortlist(cands: list[dict]) -> list[dict]:
    """一次调用筛完所有名字，只对入围的做实测——实测一个要拉 feed、取文稿，很贵。"""
...
def _name_match(want: str, got: str) -> bool:
    ...
    第一版用 10 字符前缀，于是搜「Patrick」匹配到了别的节目。第二版按最短一方的
    比例算命中数，但把 show 当停用词过滤后对方只剩一个 token，任何重叠都过
    （「Tomorrow Today」匹配上了「The Tomorrow Show」）。改用 Jaccard：
    两个名字要互相说明，不是单向包含。搜一个通用词（"Patrick"）本身就有歧义，
    这时宁可判不匹配。
    """
    import re
    ...
    if len(a & b) / len(a | b) < 0.5:
        return False
    # 交集里必须有一个"有辨识度"的词。否则「This Week in Tech」和
    # 「This Week in Startups」会靠 this/week 判成同一档 —— 那是两档不同节目。
...
SCORE_SYSTEM = """你在给一个中文播客深读站评估要不要收一档新节目。读者是中国的
创业者、投资人和工程师，他们要的是可核对的判断、机制和数字。

**只评内容价值。**能不能取到文稿、更新是否健康，由程序作为前置条件判掉了，不进你的
打分——那两项决定的是"我们能不能做"，不是"该不该做"。（第一版把它们算进分数，
结果一档有官方逐字稿的节目起步就 5 分，内容平庸也能过线，理由栏里写着"补位有限"
却给了 8 分。）
...
打分要狠。已有 60 多档信源，边际上再加一档平庸节目的代价是它会持续产生凑数内容，
而漏收一档好节目下次还能补上。8 分意味着"我会主动向同行推荐追踪这档节目"。

**把区间用起来。**如果你发现自己给一批候选打的分都一样（比如清一色 4/2/2 = 8），
那不是在评分，是在往及格线上凑——本站的成稿评审犯过一模一样的毛病，一批稿子全打 8 分，
等于把标准废掉。真正值得收的节目应该在密度上给到 5，或者在补位上给到 3。

还有一条硬规则：**如果你的理由里要写"补位有限""与现有信源重叠""部分集数偏泛"，
那补位分就不该给到 2，密度分也不该给到 4。**分数和理由必须一致；第一版出现过理由写
"补位有限"却给 8 分的情况，那种自相矛盾的评估等于没评。"""

SCORE_SCHEMA = """输出 JSON：{"density":0-5,"gap":0-3,"checkable":0-2,
"desc":"一句话中文简介，写清它到底讲什么、凭什么值得读，不超过40字",
"cat":"ai|biz|cn","tier":1|2|3,"why":"给这些分数的理由，不超过40字"}"""

# 前置条件：不满足就不打分，直接淘汰。这些是"能不能做"，不是"该不该做"。
MIN_CAND_WORDS = 3000       # 抽样文稿低于此说明是短节目，压不出有信息量的深读
MAX_CAND_STALE = 60         # 停更超过此，收了也不会更新
...
def prerequisites(c: dict) -> str | None:
    ...
    # 有音频就能转写（云端与本机都通），所以只有"短节目"这一条按字数判
...
def score_candidate(c: dict, existing_desc: str) -> dict | None:
    ...
    return {"score": round(total, 1), "density": d, "gap": g, "checkable": ck,
            ...
            "cat": r.get("cat") if r.get("cat") in ("ai", "biz", "cn") else "ai",
            "tier": r.get("tier") if r.get("tier") in (1, 2, 3) else 2,
            "why": squeeze(str(r.get("why") or ""))[:60]}


def slug_for(name: str, taken: set[str]) -> str:
    """信源 id，会出现在 /s/<id>/ 的 URL 里。

    第一版把非 ASCII 全删掉，于是「42章经」塌成 "42"、「脑放电波」塌成 "src"、
    「AI实话实说」塌成 "ai" —— 既没有辨识度又互相撞车。现在纯中文名用
    cn + 名字哈希，至少稳定且唯一。
    """
    import hashlib
    import re
...
def discover(minimum: float, dry: bool = False) -> list[dict]:
    ...
    def is_dup(name: str) -> str | None:
        """近重复源。「硅谷101|中国版」和已在册的「硅谷101」是同一档节目的衍生，
        只比小写全名会漏掉。"""
        for kn in known_nm:
