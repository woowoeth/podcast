...
打分基准（8 分是合格线，不是优秀线——一篇本职做到位的深读就该给 8）：
  9-10  除了做到位，还有别处看不到的判断，值得主动转给同行
  8     合格：要点是判断或机制而非转述、忠实、有具体数字、没有空话。上站
  6-7   有一项**具体可指出**的实质缺陷：写歪了原意、要点通篇转述、
        或者你能明确指出被丢掉的更值钱内容。含糊的"密度尚可"不算缺陷
  4-5   像是把节目介绍扩写了一遍，没有原文里的实质内容
  0-3   有歪曲，或者通篇空话
