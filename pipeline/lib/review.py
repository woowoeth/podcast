...
读者是要可核对信息的中文读者。他们读这篇是为了不听那两小时，所以这篇必须
把那两小时里真正值钱的东西端出来。审稿时按该集分类判断「值钱」：
ai/biz/cn 看产业与决策；ideas 看论证与可反驳结论；hist 看因果与可核史料；
parent 看可检验原则与证据；sci 看机制与能否回实验/论文。
不要用「对创业投资没用」去拦其他分类的合格稿。
...
4. 具体性 —— 两种都算具体，不要只认数字：
   · 事实型：数字、成本、时间线、人名、产品名（而不是"某公司""大幅提升"）
   · 判断型：论断说得够窄够硬，能被反驳；给出了取舍与边界条件，而不是
     "要保持长期视角"这种无法证伪的话
   一集投资哲学或方法论的访谈可以没有几个数字，只要它的每条要点都是一个
   可争辩的具体主张。反过来，堆满数字但每条要点都是"他们讨论了 X"，仍然是低分。
