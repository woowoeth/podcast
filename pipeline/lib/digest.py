...
**只承载一个论断，和标题同一个。** 它的作用是把标题展开成"为什么"，不是把这集的
其他论断也塞进来。
  ✗ AI进入智能体阶段：第一阶段领跑者只是炼油厂，不是原生应用大赢家；新平台不来自
     旧平台，组织要从岗位转向任务，最后人拼的是无中生有的创造力。
     （四个论断串成一句。读者记不住四件事，结果一件也没记住）
  ✓ 商业史上第一波跑出来的公司几乎从不是下个阶段的赢家——雅虎、AOL 都曾市值千亿。
     模型公司是提炼基础油的炼油厂，值钱的是化工厂和汽车公司。
40-80 字。其余论断留给要点，那才是它们的位置。

## 要点怎么选、怎么写

**选：不是这集的目录，是这集的意外。**
要点顺着节目章节排，等于把目录翻译一遍——读者拿不到任何他自己听一遍拿不到的东西。
一条要点要占位置，必须至少满足一条：
  · 会改变读者的判断（他原来以为 A，这集给了理由认为 B）
  · 给出了机制（为什么会这样，不只是"会这样"）
  · 带着一个具体的数字、案例或比喻，读者一周后还记得
三条都不满足的是话题名，删掉，把位置让给后半段更值钱的那条。宁可 5 条，不要 8 条
凑数。

**小标题必须是一句能被反对的话，8-18 字。** 判据同标题那节，只是更短。
  ✗ 机器人类比电器          （四字电报，读者不知道在说什么）
  ✓ 机器人还没等到自己的 T 型车时刻
  ✗ 战略是生成系统          （像个术语，没有断言）
  ✓ AI 时代的战略不能规划，只能生成
  ✗ 没有巨头安全            （语序别扭，"安全"还会被读成 security）
  ✓ 觉得自己安全的巨头最不安全
  ✗ 智能体阶段才开场
  ✓ token 成了共识，说明第一阶段已经结束

**正文先给论断，再给支撑。**
不要从框架起笔（"某人把 X 分成三个阶段：…"）——框架是最不容易记住的部分。
先写这条主张什么，再给理由、数字或比喻。

**留住说话人给的那个具体画面。** 服务器半夜崩了、快掉下桌子的模型火车、满嘴胡说的
大学生、想不起图灵测试的定义——这些是让判断粘住的东西。删掉画面只留结论，读者读完
就忘了；而画面是原文里真有的，不是你编的。

**他自己造的词要留下来。** formidable、jagged frontier、锯齿形边界、founder mode
——这类词是读者能带走的把手，第一次出现时给出原词加一句解释。

## 金句怎么选

金句不是格言，是**这段论证的承重句**：这句话不成立，那条要点就塌了。
  ✗ 听起来有智慧、但脱离上下文谁都能说的一句
  ✗ 最长的那一段，把说话人绕的弯子一起搬过来
  ✓ 那句"如果他没说这句，你就不会相信前面那条要点"的话
4-7 条，每条一到三句，尽量短（也更容易逐字复制成功）。彼此不要重复同一个意思——
两条金句说同一件事，等于浪费一条。
